...
DELIMITER $$
USE `bankcentric`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `singleRefAccountStatus`(IN p_account_status_id BIGINT)
BEGIN
	SELECT * FROM ref_account_status WHERE account_status_id = p_account_status_id;
END$$
DELIMITER ;
...
DELIMITER $$
USE `bankcentric`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `singleRoleByName`(IN p_role_name BIGINT)
BEGIN
